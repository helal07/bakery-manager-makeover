-- ============================================================================
-- Patch 22 — RBAC permission catalog sync (canonical)
-- Run on the production VPS Supabase SQL editor. Idempotent.
--
-- 1. Remaps legacy/duplicate permission keys to the current ones (keeps role
--    grants intact).
-- 2. Deletes orphan permissions for removed features (QC, Work Orders,
--    repurpose report, etc.).
-- 3. Upserts the full canonical catalog (71 keys) incl. Factory Stock and all
--    production reports.
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- 1. Legacy key remap: move role grants to the canonical key, drop the old row
-- ---------------------------------------------------------------------------
...
INSERT INTO _perm_remap(old_key, new_key) VALUES
  ...
  ('production.reports.view',         'production.reports.view'),
...
-- ensure the canonical targets exist before repointing grants
...
JOIN _perm_remap r ON r.old_key = rp.permission_key AND r.new_key <> r.old_key
ON CONFLICT DO NOTHING;

DELETE FROM public.role_permissions rp
USING _perm_remap r
WHERE rp.permission_key = r.old_key AND r.new_key <> r.old_key;

DELETE FROM public.permissions p
USING _perm_remap r
WHERE p.permission_key = r.old_key AND r.new_key <> r.old_key;

-- ---------------------------------------------------------------------------
-- 2. Canonical catalog upsert
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. Drop orphans (removed features: QC, Work Orders, repurpose report, …)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. Admin keeps everything except Access Control; Superadmin bypasses checks
-- ---------------------------------------------------------------------------
INSERT INTO public.role_permissions (role_id, permission_key)
...
-- Verify
-- SELECT module, count(*) FROM public.permissions GROUP BY 1 ORDER BY 1;
