-- Patch 39 — Fix Batch History "statement timeout" (idempotent). Run after 38.
-- The page ran the location access check once per ledger row (thousands of
-- rows). Now the access check runs ONCE at the start, then the query reads the
-- factory rows directly using the indexes from patch 36.
...
BEGIN
  IF COALESCE(auth.role(), '') <> 'service_role'
     AND NOT public.user_can_access_location(auth.uid(), NULL) THEN
    RAISE EXCEPTION 'Not authorized' USING ERRCODE = '42501';
  END IF;
WITH led AS MATERIALIZED (
