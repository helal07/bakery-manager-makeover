-- ============================================================================
-- Patch 32 — Production batch CRUD (edit / void) with permission guards
-- Run once in the production VPS Supabase SQL editor. Idempotent.
--
-- Adds:
--   * public.user_has_permission(uuid, text)      — RBAC lookup usable in RPCs
--   * public.void_production_batch(uuid, text)    — full reversal of a batch
--   * public.edit_production_batch(...)           — void + re-apply atomically
--   * permission keys production.batches.edit / production.batches.delete
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- 2. Internal reversal (no permission check — callers assert first)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Enough finished stock left to pull back?
...
-- ---------------------------------------------------------------------------
-- 4. Edit a batch — reverse then re-apply, keeping the same batch id
-- ---------------------------------------------------------------------------
...
-- Admin gets both by default; other roles must be ticked manually.
