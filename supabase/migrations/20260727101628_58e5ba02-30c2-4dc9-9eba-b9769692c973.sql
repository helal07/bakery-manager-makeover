-- ============================================================
-- Sub-Recipe (Intermediate Product) support
-- ============================================================
...
-- Make material_id nullable so a row can be either material or sub-recipe
ALTER TABLE public.recipes ALTER COLUMN material_id DROP NOT NULL;

-- Ensure exactly one of (material_id, sub_recipe_id) is set
...
-- Unique per product across either source
...
-- ---------- Updated commit_production_batch with sub-recipe expansion ----------
...
BEGIN
  ...
  -- Build aggregated raw material requirements (materialId -> total_qty)
  ...
  -- Lock + check stock for each aggregated material
  ...
  -- Deduct each material once (aggregated)
  FOR _mid, _required_qty IN
    SELECT key, value::numeric FROM jsonb_each_text(_totals)
  LOOP
